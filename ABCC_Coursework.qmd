...
Turn the .txt files into FASTA file (because the reference genome uses .fsa, in this case, it will stay as same as reference genome). The .txt files are already in FASTA format (header line starting with '\>' followed by sequence data), therefore can directly rename or copy it to convert the file. We will use a copy.
...
After running this command, an error occured -- When combining the files, the cdt-1 won't add in a new line, it just added at the last genome code of the gh1-1 sequence, that's because gh1-1.fsa doesn't end with a new line. Need to add a new line for combining.
...
The annotation file of reference genome is downloaded from the same website as the reference genome FastQ file. Here, we need to create the annotation for both gh1-1 and cdt-1.

Annotation file (.gff or .gtf) are tab delimited text files that describe the locations and structures of genes and other genomic features. This is made because when analysing the sequences data the software needs to be told:

1\. Where genes are located on each chromosome/sequence.

2\. What parts are exon, introns and CDS (coding sequences).

3\. Gene names and IDs.

Without an annotation file, the aligner won't know that what sequences contain genes, and some of the tools like HTSeq and featureCounts won't be able to count reads mapping to these genes. There are 9 tab-separated columns: Seqname, Source, feature, start, end, score, trand, frame, and attributes. To do this:
...
After making the annotation file of the insertion genome, we combine the insertion genome annotation and reference genome annotation.

```{bash}
cat BY4742_Toronto_2012_annotation.gff transgenes.gff > combined_reference_annotation.gff
```

But when checking the tail of the combined annotation file, there is some sequence information before gh1-1 and cdt-1. That’s because there is some sequences at the end of the reference one. Therefore need to remove the FASTA part in reference.
...
1\. Removing first 4 bases.

2\. Removing bases from the start of the read until the quality was greater or equal to three.

3\. Removing bases from the end of the read until the quality greater or equal to three.
...
Therefore, we looked into the first FastQC result again and decided to trim the sequences separately with different rules.

SRR1166442:

1.  Remove first four bases.
...
5.  If the average quality in the window drop below 20, trim the read from that point onward.
...
3.  Remove bases from the start of a read if their quality score is below five.
4.  Remove bases from the end of a read if their quality score is below five.
5.  If the average quality in the window drop below twenty, trim the read from that point onward.
6.  Drop reads shorter than 25 bases after trimming.
...
4.  If the average quality in the window drop below twenty, trim the read from that point onward.
...
Then we checked the FastQC result again and found that SRR1166442 and SRR1166445 still has some reads in the red region. So, we checked the location and trimmed again, this time by:
...
Error occured again: Error occured when processing GFF file (line 13170 of file /scratch/grp/msc_appbio/Group8_ABCC/reference_genome/combined_reference/cleaned.gff): Feature snR48_BY4742 does not contain a 'ID' attribute \[Exception type: ValueError, raised in count.py:76\]

This is becauase the ID is not in some of the lines. Changed to name, also got same error. Therefore, we realised that some are lacking in the insert genome and some in the reference genome. There are 2 option, one is to fill the ID or name, another is do the counting separately and add the result together to avoid editing the GFF for thousands of features and the “missing attribute” problem because each GFF uses the attribute it actually has.

We decided to run the Count Reads Per Gene separately, so we run them with the reference genome. This works because HTSeq-count counts reads per feature in a GFF file.

Think of --idattr as choosing a column name for counting: Reference GFF → Parent column Insert GFF → gene_id column HTSeq reads only that column. When merging, you just treat them all as “gene IDs” — it doesn’t matter which column was used in the original GFF.
...
This code ran without errors. But when we normalised the result from HTSeq, the output from this script couldn't work due to no gene length file. Therefore, we re-wrote the script to run the HTSeq and normalisation together.
