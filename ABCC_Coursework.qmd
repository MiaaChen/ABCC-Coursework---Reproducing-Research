...
## Statistics demonstrate that engineering transcription factors significantly improves cellobiose fermentation performance in yeast.
Then we downloaded the output from the HTSeq script and opened it in RStudio to combine the files. In this script: 1. Reads all normalised HTSeq files for Glucose and Cellobiose. 2. Removes HTSeq summary rows (\_\_\*). 3. Extracts RPKM values. 4. Combines them into one table with proper column names. 5. Calculates mean RPKM per condition. 6. Saves the final combined dataset for downstream analysis.
...
Cause we also got an Excel file from the paper, it also has means for three Glucose samples and three Cellobiose samples.
...
### Preparation of data for: ANOVA + Tukey test
...
Then we can run the ANOVA and Tukey tests. The script automates gene-wise ANOVA analyses, calculates descriptive stats, runs post-hoc Tukey tests, and produces comprehensive summary tables for all genes.
