...
```
After this command, six files has been created: 
combine_index.1.bt2, combine_index.2.bt2, combine_index.3.bt2, 
combine_index.4.bt2, combine_index.rev.1.bt2, combine_index.rev.2.bt2
...
However, this code finally produced the outcome without any error. While we processed to next step to normalised the result from HTSeq, the output from this script couldn't work. 
Therefore, we re-write whole script to run the HTSeq and normalisation together.

This is to count reads per gene using HTSeq and then normalise the counts to CPM and RPKM. 
At first we extract gene length from the annotation files (reference + inserts), then we count the reads per gene using HTSeq-count. After than we normalised the counts to CPM and RPKM. 
```{bash}
#!/bin/bash
