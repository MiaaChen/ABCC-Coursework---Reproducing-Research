...
When processing the data visualisation, the output of the above script did not include 'TDH3, FBA1, CCW12', which are shown in the paper's graph. Also, the normalised CSV files contain only 411 genes, whereas they’re supposed to contain more than 6000, so our plots wouldn’t match what's in the paper. 
...
This is a bash script that extracts gene lengths from a GFF file and saves them into a CSV using Python.
We asked to open the GFF file and read it line by line, skip when we met comments (#), lines with fewer than nine columns, features that are not gene.
```{bash}
...
After this script, we found out that we only have the cpm, not the RPKM.
Therefore, we broke down the script part by part to double-check the problem.
For extracting the gene length from the annotation file, before we used gene_id to label the row data, and then we used gene in the '--idattr' function. To make sure this is not the issue, changed to use the gene length. 
First, we calculated the gene length from the annotation file. 
Then, it extracted all gene features, calculated the length of each gene in base pairs, and saved the output as a CSV file. With columns: gene and length_bp.
```{bash}
#!/bin/bash
...
Then, we ran the HTSeq to count the alignment results and normalisation. 
In this script, we did not calculate the cpm as before; we only calculated the RPKM.
```{bash}
#!/bin/bash
#SBATCH --job-name=htseq_rpkm
#SBATCH --output=htseq_rpkm.out
...
Then we downloaded the output from the HTSeq script and went to RStudio to combine the files into one. 
In this script:
1. Reads all normalised HTSeq files for Glucose and Cellobiose.
2. Removes HTSeq summary rows (__*).
3. Extracts RPKM values.
4. Combines them into one table with proper column names.
5. Calculates mean RPKM per condition.
6. Saves the final combined dataset for downstream analysis.
```{r}
getwd()
...
To run the statistical part at the end of the paper, we need to restructure the file.
The script takes your wide-format RPKM data, reshapes it into a long/tidy format, extracts experimental metadata (strain, replicate, condition), converts relevant columns to factors, and saves it as a new CSV for ANOVA analysis.
```{r}
getwd()
library(tidyr)
library(dplyr)
...
Then we can run the ANOVA test and Tukey test.
The script automates gene-wise ANOVA analyses, calculates descriptive stats, runs post-hoc Tukey tests, and produces comprehensive summary tables for all genes.
```{r}
library(tidyverse)
