...
For the insertion sequences (gh1-1 & cdt-1); get onto several website on ncbi then copy and paste onto txt files.
...
Right now, the example of the txt files. (need to remove the carriage returns, tabs, and space from the sequence part)

```{bash}
#keep the first line unchanged, and remove the newlines, carriage retuns, tab, and space from the rest.
(head -n 1 gh1-1.txt && tail -n +2 gh1-1.txt | tr -d '\n\r\t ') > gh1-1_clean.txt
( head -n 1 gh1-1.txt && tail -n +2 gh1-1.txt | tr -d '\n\r\t ' ) > cdt-1_clean.txt
```

Turn the txt files into fasta file (cause the reference genome use .fsa, in this case, will stay as same as reference genome).The txt files are already in fasta formate (has header line starting with '\>' follwed by sequence data, therefore can directly rename or copy it to convert the file). To keep all be recorded, will use copy.
...
After running this command, an error occur -- When combining the files, the cdt-1 won't add in a new line, it just added at the last genome code of the gh1-1 sequence, that's because gh1-1.fsa doesn't end with a new line. Need to add a new line for combing.
...
The annotation file of reference genome is downloaded from the same website as the reference genome fastq file. Here need to create the annotation for both gh1-1 and cdt-1.

Annotation file (.gff or .gtf) is tab delimited text files, that describe the locations and structures of genes and otehr genomic features. The reason for making it, is when analysing the sequences data, need to tell the software that 1. where genes are located on each chromosome/sequence. 2. What parts are exon, introns, CDS (coding sequences). 3. Gene names and IDs. Without an annotation file, the aligner won't knwo that what sequences contain genes, and some of the tools like HTSeq and featureCounts won't be able to count reads mapping to these genes. There has 9 tab-separated columns: Seqname, Source, feature, start, end, score, trand, frame, and attributes. To do this:

First, need to calculate the length of sequences

```{bash}
GH1_LEN=$(grep -v "^>" gh1-1.fsa | tr -d '\n'|wc -c)
CDT_LEN=$(grep -v "^>" cdt-1.fsa | tr -d '\n'|wc -c)

# Check the length before make the annotation 
...
After making the annotation file of the insertion genome, combine the insertion genome annotation and reference genome annotation.

```{bash}
cat BY4742_Toronto_2012_annotation.gff transgenes.gff > combined_reference_annotation.gff
```

There has a problem -- when checking the tail of the combined annotation file, there has some sequence information before gh1-1 and cdt-1. That’s because there has some sequence at the end of the reference one. Therefore need to remove fasta part in reference.
...
Then we have a whole annotation GFF file for all sequences.
...
At first we trimmed by: 1. Remove first 4 bases. 2. Remove bases from the start of the read until quality greater or equal to three. 3. Remove bases from the end of the read until the quality greater or equal to three. 4. cut the reads when the average drops below Q20. 5. If the read is shorter than 30 bp, drop the read. After trimmed the sequences, we made another fastqc test to check the quality after trimming is as good as we want.
...
Then we check the fastqc_post result, still have some in red region.

-   SRR1166442: Still has a peak went down to red region.

<!-- -->

-   SRR1166446: Still has a peak went down to red region.

Therefore, we looked into the first fastqc result again, and decided to trim the sequences separetely with different rules.

SRR1166442:

1.  Remove first 4 bases.
...
5.  If the average quality in the window drop below twenty, it trims the read from that point onward.
...
4.  If the average quality in the window drop below twenty, it trims the read from that point onward.
...
Then we checked the fastqc result again, found out that SRR1166442 and SRR1166445 still has some part in red region. So we checked the location and then trimmed again, this time trimmed by:
...
First need to build a Bowtie2 index for the reference genome, becuase Bowtie2 does not align reads directly to a raw Fasts file.

```{bash}
bowtie2-build combined_reference.fsa combined_reference.index 
```

After this command, six files has been created: combine_index.1.bt2, combine_index.2.bt2, combine_index.3.bt2, combine_index.4.bt2, combine_index.rev.1.bt2, combine_index.rev.2.bt2

Then aligning multiple trimmed FASTQ files to reference genome, converting the results into compressed and indexed BAM files:
...
echo "Starting to count the read per gene"
...
At first time to run the code, got error: error i got: Error occured when processing GFF file (line 10142 of file /scratch/grp/msc_appbio/Group8_ABCC/reference_genome/combined_reference/combined_reference_annotation.gff): unmatched quote \[Exception type: ValueError, raised in \_HTSeq.pyx:1597\];

That is because in annotation file, got a unexpected ‘“‘. Need to remove it !!

```{bash}
# First check how it looks like in the 1042 line.
nano +10142 combined_reference_annotation.gff 
# See there has a unpaired quotation marks, then remove it 
sed '10142s/"//g' combined_reference_annotation.gff > cleaned.gff
# Right now check there has no more unpaired quotation marks 
...
Error occured again: Error occured when processing GFF file (line 13170 of file /scratch/grp/msc_appbio/Group8_ABCC/reference_genome/combined_reference/cleaned.gff): Feature snR48_BY4742 does not contain a 'ID' attribute \[Exception type: ValueError, raised in count.py:76\] This is becuase the ID lack in some of the lines. Changed to name, also got same error. Therefore, we released that some is lack in the insert genome some is lack in the reference genome. There has 2 option, one is to fill the id or name, another is do the counting stuff seperately and add the result tgt, this will avoid editing the GFF for thousands of features and the “missing attribute” problem because each GFF uses the attribute it actually has. We decidede to run the Count Reads Per Gene separately, so run with the reference genome It is fine, cause HTSeq-count counts reads per feature in a GFF file. Think of --idattr as choosing a column name for counting: Reference GFF → Parent column Insert GFF → gene_id column HTSeq reads only that column. When merging, you just treat them all as “gene IDs” — it doesn’t matter which column was used in the original GFF.
...
However, this code finally produced the outcome without any error. While we processed to next step to normalised the result from HTSeq, the output from this script couldn't work due to no gene length file. Therefore, we re-write whole script to run the HTSeq and normalisation together.

This is to count reads per gene using HTSeq and then normalise the counts to CPM and RPKM. At first we extract gene length from the annotation files (reference + inserts), then we count the reads per gene using HTSeq-count. After than we normalised the counts to CPM and RPKM.
